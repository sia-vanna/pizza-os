...
void putchar(char c, unsigned char color=0x0F){
  if(c=='\n'){
    ...
    if(cy>=25){
      cy=24;
      for(int y=1;y<25;y++)
        for(int x=0;x<80;x++) vga[(y-1)*80+x]=vga[y*80+x];
...
void print_int(int n, unsigned char color){
  ...
  if(neg) rev[r++]='-';
  char buf[12]; int i=0; while(r--) buf[i++]=rev[r]; buf[i]=0;
  print(buf, color);
}
void print_dec(int n, unsigned char color=0x0F){ print_int(n,color); }

void print_hex(uint64_t v, unsigned char color){
  char buf[17];
  for(int j=15;j>=0;j--){ int nib=v&0xF; buf[j]=nib<10?'0'+nib:'a'+nib-10; v>>=4; }
  buf[16]=0; print(buf,color);
}
...
extern "C" void kernel_main(uint32_t magic, uint32_t mb_info_ptr){
  ...
  print("[OK] ",0x0A); print("VGA 0xB8000 + SERIAL 0x3F8 initialized\n",0x07);
  print("[OK] ",0x0A); print("No telemetry. No account. No feed.\n\n",0x08);

  bool is_mb2 = (magic==0x36d76289);
  print("[INFO] boot: ",0x0B); print(is_mb2?"multiboot2 (GRUB)\n":"PVH / direct (fallback)\n",0x07);

  bool real_map = mm_init(magic, mb_info_ptr);
  if(real_map) print("[OK] Real memory map found\n",0x0A);
  else print("[OK] Using fallback allocator 16MB-64MB\n",0x0A);
  mm_print_map();

  print("\n[INIT] ",0x0B); print("fs init (user db)\n",0x07);
  fs_init(mb_info_ptr);
  fs_list();

  auto* pw = fs_open("/etc/passwd");
  if(pw){ print("\n[DB] /etc/passwd:\n",0x0E); print((char*)pw->data,0x08); print("\n",0x07); }

  print("\n[TEST] Bump allocator:\n",0x0B);
  void* p1=mm_alloc_pages(1); void* p2=mm_alloc_pages(1);
  ...
  // shell stays alive
  while(1){
    print("\n[pizza-os] shell idle — type reboot via web reset\n",0x08);
