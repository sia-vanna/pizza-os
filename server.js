...
wss.on('connection', ws => {
  ...
  q.stderr.on('data', d => { console.log('[QEMU-ERR]', d.toString()); if (ws.readyState === 1) ws.send(d.toString()); });
  q.on('close', c => { console.log('[QEMU] exit', c); try{ ws.send(`\n--- qemu exit ${c} ---\n`) }catch{} ws.close(); });
  ws.on('message', m => { if (q.stdin.writable) q.stdin.write(m.toString().replace(/\x7f/g, '\x08')); });
