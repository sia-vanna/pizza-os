...
wss.on('connection',ws=>{
  if(!fs.existsSync(BIN)){ ws.send('BIN missing, POST /api/build first\n'); return ws.close(); }
  // one qemu per connection — droplet has no KVM so force tcg
  ...
  ws.on('message',m=>{
    ...
    let s=m.toString().replace(/\x7f/g,'\x08'); // fix backspace
    q.stdin.write(s);
  });
  ws.on('close',()=>{ q.kill('SIGKILL'); console.log('[WS] closed'); });
});

server.listen(3000,()=>console.log('🍕 http://localhost:3000 BIN='+BIN));